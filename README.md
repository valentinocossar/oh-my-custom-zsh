...
- `oh-my-custom-zsh.zsh`: `ZSH_CUSTOM` loader - sources the `*.zsh` partials from `includes/zsh/`
...
- **Add or remove a package**: install/uninstall it normally with `brew`/`mas`, then regenerate the Brewfile from the current system state: `brew bundle dump --global --force --taps --brews --casks --mas`. Do not hand-edit the ordering of the file — always regenerate it with `--force` so the diff only shows the actual package change, then review with `git diff` before committing.
...
To bump everything at once, run `mise run bump` — a wrapper for `mise upgrade --bump` that skips `php`, which is a `mise link` to the Homebrew keg and must not be rebuilt from source.
